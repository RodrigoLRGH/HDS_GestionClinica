...
namespace Presentacion.Controllers
{
    ...
    public class HistorialController : ControllerBase
    {
        ...
        [HttpGet("historia/paciente/{pacienteId}")]
        public async Task<IActionResult> ObtenerHistoriaPorPaciente(int pacienteId)
        {
            var resultado = await _historiaClinicaService.ObtenerTodosAsync();
            if (!resultado.Exitoso) return BadRequest(resultado.Mensaje);
            var historia = resultado.Datos?.FirstOrDefault(h => h.IdPaciente == pacienteId);
            if (historia == null) return NotFound("Historia clínica no encontrada.");      
            return Ok(historia);
        }
